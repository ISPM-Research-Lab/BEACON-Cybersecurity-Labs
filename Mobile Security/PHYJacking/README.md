# [BEACON Labs - Mobile Security: Android Physical Input Hijacking (PHYjacking) Attack Lab](https://sites.google.com/boisestate.edu/beacon-labs/mobile-security-labs/phyjacking)

## Type

Mobile Security Lab

## Overview

Nowadays, most mobile devices are equipped with various hardware interfaces such as touchscreen, fingerprint scanner, camera, and microphone to capture inputs from the user. Many mobile apps use these physical interfaces to receive user-input for authentication/authorization operations, including one-click login, fingerprint-based payment approval, and face/voice unlocking.

This exercise implements what is known as Physical Input Hijacking to create unintended authorization with zero needed permissions. Furthermore, we explore weaknesses in existing protection schemes and pitfalls in app implementations. This lab will instruct you on how to make an unsuspecting user provide authorization on an unintended fund transfer through a fake banking application.

## Lab Materials

- [Student Lab Materials](/Mobile%20Security/PHYJacking/Student%20Lab%20Materials/):
    - [Instruction Document](/Mobile%20Security/PHYJacking/Student%20Lab%20Materials/Beacon_MS_AndroidPHYjacking_Lab.pdf)
    - [Report Template](/Mobile%20Security/PHYJacking/Student%20Lab%20Materials/Lab_Report_Template.docx)
    - [Project Folder](https://drive.google.com/file/d/1ACYQ1qE1wGhfaAeTw7Pt6uyXI6C_BEKZ/view?usp=share_link)
    - **Android Studio**: The Android Studio IDE can be downloaded from the [official Android Developer website](https://developer.android.com/studio).
        - Alternatively, Android Studio can be downloaded from [JetBrains Toolbox](https://www.jetbrains.com/toolbox-app/).
- [Instructor Lab Materials](/Mobile%20Security/PHYJacking/Instructor%20Lab%20Materials/):
    - [Instructor Manual](/Mobile%20Security/PHYJacking/Instructor%20Lab%20Materials/[PHY]%20Lab%20Report%20Teacher.docx)
    - [Instructor Solution Project](https://drive.google.com/file/d/1Rgc14y8aZA1OYiDZ2vHCC7cxBc-o4Hir/view?usp=drive_link)
    - [LaTeX](/Mobile%20Security/PHYJacking/Instructor%20Lab%20Materials/LaTeX/)
- [Auxiliary Learning Materials](/Mobile%20Security/PHYJacking/Auxiliary%20Learning%20Materials/):
    - [Setup Document](/Mobile%20Security/PHYJacking/Auxiliary%20Learning%20Materials/Beacon_Workshop_MS_Setup_Instructions.pdf)
    - [Instruction Document](/Mobile%20Security/PHYJacking/Auxiliary%20Learning%20Materials/Beacon_Workshop_MS.pdf)
    - [Slides](/Mobile%20Security/PHYJacking/Auxiliary%20Learning%20Materials/BEACON-Labs_Workshop-MS.pptx)
    - [Project](https://drive.google.com/file/d/17KuEH9aQtD4UuVXMFIOAWq97-_Z-CzXk/view?usp=sharing)
    - [LaTeX](/Mobile%20Security/PHYJacking/Auxiliary%20Learning%20Materials/LaTeX/):
        - [Setup Document](/Mobile%20Security/PHYJacking/Auxiliary%20Learning%20Materials/LaTeX/Setup/)
        - [Instruction Document](/Mobile%20Security/PHYJacking/Auxiliary%20Learning%20Materials/LaTeX/Instruction-Document/)

## Source Paper

This lab is based [on a paper from the 2022 NDSS Symposium](https://www.ndss-symposium.org/wp-content/uploads/2022-97-paper.pdf).

For additional information, visit the [NDSS website](https://www.ndss-symposium.org/ndss-paper/auto-draft-187/).

## Complementary Materials

[![Android PHYJacking video by Gunnar Vittrup.](https://img.youtube.com/ZrGEqWL43wQ/hqdefault.jpg)](https://www.youtube.com/watch?v=ZrGEqWL43wQ)

[![Semi-Separating Equilibrium video by William Spaniel](https://img.youtube.com/h3JQYd4BFGk/hqdefault.jpg)](https://www.youtube.com/watch?v=h3JQYd4BFGk)

## Suggested Time

- Supervised (guided lab session): **3.75 hours**
- Unsupervised (take-home project): **1.5 week**
- Level of Difficulty: 2-3

## Feedback

If you have any feedback to provide on this lab please [let us know](https://docs.google.com/forms/d/1ERz-IhIprUOmSpoK3ARNiLZ9Z9JhZgCl4HE51Lb4rDs/edit?usp=sharing).
