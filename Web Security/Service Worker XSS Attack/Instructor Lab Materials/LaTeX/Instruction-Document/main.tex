...
\ProjectFullTitle{Beacon Network Security Labs: Service Worker Cross-Site Scripting Attacks}
...
In this lab, you will learn about the Service Worker Cross-Site Scripting (SW-XSS) attack and how to defend against it.
...
After completing this lab you should be able to understand a Cross Site Scripting hack, how it works, and how to defend against it, understand sources and sinks, and be able to implement Cross Site Scripting and Service Worker Cross Site Scripting attacks. 
...
A provided zipped folder contains all of the necessary files to run and complete this lab. Your machine should have at least 8 GB RAM, a x86\_64 CPU architecture, at least a 2nd generation Intel Core or AMD CPU with support for a Windows Hypervisor / ARM-based chips, and at least 17 GB of available disk space.

Additionally, you will need a Hypervisor such as VirtualBox, which we will use in the setup document.

To setup your lab environment, please follow the provided Setup document.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Lab Task 1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Task Set 1: DOM-XSS}\label{sec:Task 1: DOMXSS}
\addcontentsline{toc}{section}{Task Set 1: DOM-XSS}
...
\subsection*{Task 1.1: Explaining DOM-XSS}
\addcontentsline{toc}{subsection}{Task 1.1: Explaining DOM-XSS}
...
Now that you have a basic overview of the theory let's jump into the lab to see this attack in action.
...
1.1.1 - Start your VM following the setup document and use the password "swxss" to login. Open the Firefox web browser and click on the bookmark "DOM-XSS." After clicking inspect and reading the code, determine what is the source and what is the sink? Enter your answer into your lab report.\\
...
Copy your URL into your lab report along with a screenshot showing if an alert appears. Should an alert appear, be sure that the screenshot shows the alert and the message it displays.
\end{mdframed}

\subsection*{Task 1.2: Implementing DOM-XSS}
\addcontentsline{toc}{subsection}{Task 1.2: Implementing DOM-XSS}
...
1.2.1 - Craft a new URL using an \texttt{<img>} tag and an image hosted on the site \url{https://www.attacker.com}. In your lab report, include the URL as text and paste a screenshot of the web page after executing the URL.
\end{mdframed}
...
\section*{Task Set 2: Using Service Workers for Cross-Site Scripting}\label{sec:Task 3: Using Service Workers for Cross-Site Scripting}
\addcontentsline{toc}{section}{Task Set 2: Using Service Workers for Cross-Site Scripting}

In this section, we will discuss the lab's main focus, the SW-XSS attack. We will cover what a service worker is, the service worker life cycle, the service worker's source and sinks, and how to execute an SW-XSS attack.

A basic understanding of service workers is crucial to completing this lab because they are attack vectors for the SW-XSS attack. To be ethical hackers and/or security minded developers, we need to play the role of attackers. So donning the attacker thinking cap, let’s look at service workers through the eyes of an attacker.

\subsection*{Task 2.1: Service Worker Overview}
\addcontentsline{toc}{subsection}{Service Worker Overview}

First, what are service workers? Service workers are web workers what are JavaScript files running in a separate thread on the browser. Service workers are a subclass of web workers with powerful capabilities such as network traffic interception, persistence across sessions, and instant push notifications. 

What is important to know for this lab is that service workers have a life cycle where they are registered, installed, activated, and finally destroyed. Knowledge of this life cycle is pivotal in implementing the SW-XSS attack, as an attacker needs to place an event listener during the installation phase. Once installed, the service worker denies any further additions to the service worker function file essentially locking it from outside changes.

\subsection*{Service Worker Lifecycle}
\addcontentsline{toc}{subsection}{Service Worker Lifecycle}

Let’s get a general idea of what each step does before diving into the code.

\subsubsection*{Registration}
\addcontentsline{toc}{subsubsection}{Registration}
...
\addcontentsline{toc}{subsubsection}{Activation}
...
\subsection*{Explaining SW-XSS}
\addcontentsline{toc}{subsection}{Explaining SW-XSS}
...
\subsubsection*{SW-XSS Sinks}
\addcontentsline{toc}{subsubsection}{SW-XSS Sinks}
...
\subsection*{Task 2.3: Implementing SW-XSS}
\addcontentsline{toc}{subsection}{Task 2.3: Implementing SW-XSS}

In the next sections, we will take our knowledge of DOM-XSS and service workers and implement our own SW-XSS attack. To assist you, we created an accompanying video demonstrating debugging processes. You can find this video on this lab's webpage.

\subsubsection*{Crafting the URL}
\addcontentsline{toc}{subsubsection}{Crafting the URL}
...
\subsubsection*{Reloading Service Workers}
\addcontentsline{toc}{subsubsection}{Reloading Service Workers}
...
2.3.1 - Open a new Firefox tab to the "SW-XSS" bookmark. Inspect the web page, click the debugger tab, and click on \texttt{sw.js} on the far right. Report the source and the sink in your lab report.\\
2.3.2 - Open a terminal, and change the directory to \texttt{/var/www/html}. Create a new file called \texttt{sw\_fn.js} with a personalized console message. Remove the old service worker in your browser and reload the SW-XSS page. Come back to the debugging page. Screenshot the resulting console log and add it to your lab report.

\end{mdframed}

\subsubsection*{Implement Event Handlers}
\addcontentsline{toc}{subsubsection}{Implement Event Handlers}
...
Opening the \texttt{sw\_fn.js} file in an editor such as `vi' or `nano' will reveal various functions as seen in Figure~\ref{fig:unfinished sw-fn.js} on the next page.
...
2.3.3 - In the file \texttt{sw\_fn.js}, edit the functions to add the install, activate, and fetch handlers. Add a console.log to each function and screenshot your resulting functions and the log output.\\
2.3.4 - In the same file, edit the fetch handler function to replace an HTML element on the SW-XSS page like we did in the DOM-XSS section. Screenshot the resulting function and page.
...
%%% Game Theory
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Task Set 3: Game Theory}\label{sec:GameTheory}
...
\subsection*{Game Theory Assumptions}
\addcontentsline{toc}{subsection}{Game Theory Assumptions}
...
\subsection*{Task 3.1: Game Theory Payoffs}
\addcontentsline{toc}{subsection}{Task 3.1: Game Theory Payoffs}
...
\addcontentsline{toc}{subsection}{Strategies}
...
3.1.1 - Calculate the hacker's expected utility for this game and record it in your lab document. What does the expected utility tell about the hacker's decision?\\
3.1.2 - Calculate the expected utility for p2 if p1 chooses the forget option. Record your answer in the lab document. Does this game have an equilibrium? Explain your reasoning.
...
Submit a lab report utilizing the provided template to your instructor, including screenshots, thoughts, challenges, and lessons learned from the lab. Furthermore, zip and attach your project files with the relevant JavaScript files for submission.
...
In this lab, you learned about the cutting-edge SWXSS attack. You learned how the attack works, what causes this attack, and how to nullify it. This lab is a good, practical demonstration of how simple oversights can lead to devastating issues.
