...
This activity in the workshop will be done using a single virtual machine that will be provided to you. Said image can be downloaded from \href{https://drive.google.com/file/d/1FWWNZCXi8gSi9SsbiBaMJNuyTAFvbDps/view?usp=sharing}{this Google Drive link}.

\url{https://drive.google.com/file/d/1FWWNZCXi8gSi9SsbiBaMJNuyTAFvbDps/view?usp=sharing}

\paragraph*{Steps:}

\begin{enumerate}
    \item Use the above link to open Google Drive to two virtual machine image file.
    \item Download the image file. The file is 5.54 GB in size so the download may take a while.
    \item Once the images has been downloaded, it can be imported into VirtualBox by navigating to File > Import Appliance and selecting the .ova image file that was downloaded. Visual instructions can be seen in Figures~\ref{fig:import-appliance}, \ref{fig:import-file-button}, \ref{fig:import-file-picker}, \ref{fig:import-after-file-picker}, and~\ref{fig:import-finish}.
    ...
    \begin{figure}[!htbp]
        ...
        \caption{Select the file button to choose the downloaded image file.}
        \label{fig:import-file-button}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.75\linewidth]{graphics/net-import-file-picker.png}
        \caption{Choose the downloaded .ova file to import.}
        \label{fig:import-file-picker}
    \end{figure}
    
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.75\linewidth]{graphics/net-import-after-filepicker.png}
    ...
    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.75\linewidth]{graphics/net-import-finish.png}
        \caption{Final confirmation. Your file paths and names may differ slightly.}
        \label{fig:import-finish}
    \end{figure}
    
    \item Once the image has been imported to VirtualBox, continue to the final section, ``Verifying Import.''
\end{enumerate}

\subsubsection*{Verifying Import}
\addcontentsline{toc}{subsubsection}{Verifying Import}

\paragraph*{Steps:}

\begin{enumerate}
    \item Start the virtual machine.
    \item Use ``middlebox'' as the username and password to log in.
    \item Open a terminal by either selecting the terminal application on the Desktop or App Launcher or executing the keyboard shortcut Ctrl+t.
    \item Verify that Java can be found and run by executing the command \texttt{java --version}.
    \item If Java was found, you are done, continue to Step 7. If not, continue to the next Step.
    \item Download Java by executing the command \texttt{sudo apt-get install openjdk-17-jdk}. Use ``middlebox'' as the password if prompted. After the download is complete, repeat Step 4.
    \item Congratulations, you've successfully imported the virtual machine image and are now ready for the Network Security workshop segment of the BEACON Labs Workshop. Thank you for coming prepared! If you haven't already, please follow the instructions for the Web and Mobile Security material setups. We look forward to seeing you in Boise for the workshop!
\end{enumerate}
